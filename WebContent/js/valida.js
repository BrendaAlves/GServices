				// >>>> Exibe e esconde cadastro Salão/Cliente

function doCadastro(exibe_cad, esconde_cad){ 
	var exibe = document.getElementById(exibe_cad).style.display; 
	
	if (exibe == "none"){
		document.getElementById(exibe_cad).style.display = 'block'; 
		document.getElementById(esconde_cad).style.display = 'none';
	}
	else{ 
...
let parent = document.querySelector("#nav-menu");

parent.addEventListener('click', (event) => {
   if (event.target.className === "li") {
     event.target.style.display = "block";

       let allOptions = parent.querySelectorAll(".li");
       ...
       for (let option of allOptions) {
           if (option != event.target) {
			...
			// >>>> Exibe e esconde senha
$(document).ready(function(){
	$('#exibeSenha').on('click', function(){
		var passwordField = $('#cad_senha');
...
function validarEmail(field) {
	...
	&& (dominio.lastIndexOf(".") < dominio.length - 1)) {
		document.getElementById("emailResponse").innerHTML = "<font color='green'>✓</font>";
	} 
	else {
		document.getElementById("emailResponse").innerHTML = "<font color='red'>Email inválido!</font>";	
	}
}

	
				// >>>> VALIDA SE SENHA E CONFIRMA SENHA SÃO IGUAIS
function compareSenha(){
	function valideSenha(){
	   senha = document.getElementsByName('cad_senha').value;
	   senhaConf = document.getElementsByName('cad_senhaConf').value;
...
function mtel(v){
    ...
    v=v.replace(/(\d)(\d{4})$/,"$1-$2");		// Coloca hífen entre o quarto e
												// o quinto dígitos
    return v;
}
function id( el ){
...
window.onload = function(){
	// ID AQUI _CadSalao
	id('telefone_Sal').onkeyup = function(){
		mascara(this, mtel);
	}
} 
window.onload = function(){
	// ID AQUI _CadCliente
	id('telefone_Cli').onkeyup = function(){
		mascara(this, mtel);
	}
} 
window.onload = function(){
	// ID AQUI _CadFornecedor
	id('telefone_Fornec').onkeyup = function(){
		mascara(this, mtel);
	}
}
...
cpfCheck = function (el){
	// ID AQUI
	document.getElementById('cpfResponse').innerHTML = is_cpf(el.value)? '<h6>CPF <a style="color:green">✓</a></h6>' : '<h6 style="color:yellow"><i>CPF Inválido!</i></h6>';
    if(el.value=='') document.getElementById('cpfResponse').innerHTML = '';
}
cNPJCheck = function (el){
	// ID AQUI
	document.getElementById('cnpjResponse').innerHTML = is_cnpj(el.value)? '<h6>CNPJ <a style="color:green">✓</a></h6>' : '<h6 style="color:yellow"><i>CNPJ Inválido!</i></h6>';
...
*/
					// >>>> SCRIPT EXIBE UMA ABA E ESCONDE A OUTRA
/*function id(el) {
	return document.getElementById(el);
}
...
(function(a,b){
	...
	var j=function(){
		var b={
			...
			contains:function(a){
				for(var b=a&&a.valueOf(),c=0,d=this.length;d>c;c++)if(this[c].valueOf()===b)return c;return-1
			},
			remove:function(a){
				this.splice(a,1)
			},
	...
	(),k=function(b,c){a(b).data("datepicker",this),
	...
	k.prototype={constructor:k,_resolveViewName:
		function(a,c){
			return 0===a||"days"===a||"month"===a?0:1===a||"months"===a||"year"===a?1:2===a||"years"===a||
					"decade"===a?2:3===a||"decades"===a||"century"===a?3:4===a||"centuries"===a||"millennium"===a?4:c===b?!1:c
		},_check_template:
		function(c){
			try{
				if(c===b||""===c)return!1;if((c.match(/[<>]/g)||[]).length<=0)return!0;
				var d=a(c);
				return d.length>0
			}
			catch(e){
			...
			function(b){
				...
				e.startDate!==-(1/0)&&(e.startDate?e.startDate 
					instanceof Date?e.startDate=this._local_to_utc(this._zero_time(e.startDate)):e.startDate=r.parseDate(e.startDate,g,e.language,
				e.assumeNearbyYear):e.startDate=-(1/0)),
				e.endDate!==1/0&&(e.endDate?e.endDate 
						instanceof Date?e.endDate=this._local_to_utc(this._zero_time(e.endDate)):e.endDate=r.parseDate(e.endDate,g,e.language,
				e.assumeNearbyYear):e.endDate=1/0),
				...
				var h=String(e.orientation).toLowerCase().split(/\s+/g),i=e.orientation.toLowerCase();
					...
					e.orientation={x:"auto",y:"auto"},i&&"auto"!==i)if(1===h.length)
				switch(h[0]){
					case"top":case"bottom":e.orientation.y=h[0];
						break;
					case"left":case"right":e.orientation.x=h[0]
				}
			...
			_buildEvents:function(){
				...
				this.isInput?this._events=[[this.element,b]]:this.component&&this.hasInput?this._events=[[this.inputField,b],
				[this.component,{
			...
			show:function(){
				...
				this._trigger("show"),(window.navigator.msMaxTouchPoints||"ontouchstart"in document)&&this.o.disableTouchKeyboard&&a(this.element).blur(),
				this)
			},
			hide:function(){
			...
			paste:function(b){
				...
				this.setDate(c),
				this.update(),
				b.preventDefault()},_utc_to_local:function(a){
					return a&&new Date(a.getTime()+6e4*a.getTimezoneOffset())
				},
				_local_to_utc:function(a){
					return a&&new Date(a.getTime()-6e4*a.getTimezoneOffset())},_zero_time:function(a){
				...
				var j=Math.max.apply(Math,i)+this.o.zIndexOffset,
					...
					n=k.left-h.left,o=k.top-h.top;"body"!==this.o.container&&(o+=g),
				...
				if("auto"===q&&(p=-g+o-c,q=0>p?"bottom":"top"),
				...
				this.dates.length?this.viewDate=new 
						...
						Date(this.o.endDate):this.viewDate=this.o.defaultViewDate,d?this.setValue():c.length&&String(b)!==String(this.dates)&&this._trigger("changeDate"),
				!this.dates.length&&b.length&&this._trigger("clearDate"),
					this.fill(),this.element.change(),
					this
				},
				fillDow:function(){
					...
					c+='<th class="cw">&#160;</th>');
					b<this.o.weekStart+7;)c+='<th class="dow',
				...
				fillMonths:function(){
					for(var a=this._utc_to_local(this.viewDate),
						b="",c=0;12>c;){
				...
				getClassNames:function(b){
					var c=[],d=this.viewDate.getUTCFullYear(),
					...
					this.dateIsDisabled(b)&&c.push("disabled",
						"disabled-date"),-1!==a.inArray(b.getUTCDay(),
					this.o.daysOfWeekHighlighted)&&c.push("highlighted"),
					this.range&&(b>this.range[0]&&b<this.range[this.range.length-1]&&c.push("range"),
							-1!==a.inArray(b.valueOf(),
				...
				_fill_yearsView:function(c,d,e,f,g,h,i,j){
					...
					u===b?u={}:"boolean"==typeof u?u={enabled:u}:"string"==typeof 
					u&&(u={classes:u}),
				...
				fill:function(){
					...
					g=f.getUTCFullYear(),
					h=f.getUTCMonth(),
					i=this.o.startDate!==-(1/0)?this.o.startDate.getUTCFullYear():-(1/0),j=this.o.startDate!==-(1/0)?this.o.startDate.getUTCMonth():-(1/0),
					...
					this.fillMonths();
					var p=c(g,h-1,28),
						s=r.getDaysInMonth(p.getUTCFullYear(),
						p.getUTCMonth());
						p.setUTCDate(s),
					...
					for(var u,v=[];
						...
						if(p.getUTCDay()===this.o.weekStart&&(v.push("<tr>"),
							...
							e===b?e={}:"boolean"==typeof e?e={enabled:e}:"string"==typeof 
							e&&(e={classes:e}),
				...
				if(a.each(this.dates,
					...
					(i>g||g>k)&&B.addClass("disabled"),g===i&&B.slice(0,j).addClass("disabled"),g===k&&B.slice(l+1).addClass("disabled"),
				...
				a.each(B,
					function(c,d){
							...
							f===b?f={}:"boolean"==typeof f?f={enabled:f}:"string"==typeof 
							f&&(f={
								classes:f
							}),
		...
		click:function(b){
			...
			e.hasClass("new")&&(11===i?(i=0,h+=1,j=!0,k=!0):(i+=1,j=!0)),
				this._setDate(c(h,i,g)),k&&this._trigger("changeYear",
				this.viewDate),j&&this._trigger("changeMonth",this.viewDate)),
			e.hasClass("month")&&(this.viewDate.setUTCDate(1),
			g=1,i=e.parent().find("span").index(e),
			h=this.viewDate.getUTCFullYear(),
			...
			g=1,i=0,h=parseInt(e.text(),10)||0,
		...
		weekOfDateIsDisabled:function(b){
			return-1!==a.inArray(b.getUTCDay(),
		...
		keydown:function(a){
			...
			switch(a.keyCode){
				...
				break;
					...
					a.shiftKey?(c=this.moveAvailableDate(e,b,"moveMonth"),
					c&&this._trigger("changeMonth",this.viewDate)):
					37===a.keyCode||39===a.keyCode?c=this.moveAvailableDate(e,b,"moveDay"):
					this.weekOfDateIsDisabled(e)||(c=this.moveAvailableDate(e,b,"moveWeek")):
					1===this.viewMode?((38===a.keyCode||40===a.keyCode)&&(b=4*b),
					c=this.moveAvailableDate(e,b,"moveMonth")):
					2===this.viewMode&&((38===a.keyCode||40===a.keyCode)&&(b=4*b),
	...
	var l=function(b,c){
		...
		delete c.inputs,n.call(a(this.inputs),c).on("changeDate",
		a.proxy(this.dateUpdated,this)),
	...
	var o=a.fn.datepicker.defaults={
		assumeNearbyYear:!1,
		autoclose:!1,
		beforeShowDay:a.noop,beforeShowMonth:a.noop,
		beforeShowYear:a.noop,
		beforeShowDecade:a.noop,
		beforeShowCentury:a.noop,
		calendarWeeks:!1,
		clearBtn:!1,
		toggleActive:!1,daysOfWeekDisabled:[],
	...
	r={
		...
		parseDate:function(e,f,g,h){
			function i(a,b){
				return b===!0&&(b=10),
				100>a&&(a+=2e3,a>(new Date).getFullYear()+b&&(a-=100)),a
			}
			function j(){
			...
			u={
				yesterday:"-1d",today:"+0d",tomorrow:"+1d"
			};
			if(/^[\-+]\d+[dmwy]([\s,]+[\-+]\d+[dmwy])*$/.test(e)){
				for(e=new Date,n=0;n<s.length;n++)l=p.exec(s[n]),
					...
					o=t[l[2]],
					e=k.prototype[o](e,m);
				return c(e.getUTCFullYear(),
					e.getUTCMonth(),
					e.getUTCDate())
			}
			if("undefined"!=typeof u[e]&&(e=u[e],
			s=e.match(/([\-+]\d+)([dmwy])/g),/^[\-+]\d+[dmwy]([\s,]+[\-+]\d+[dmwy])*$/.test(e))){
				for(e=new Date,n=0;n<s.length;n++)l=p.exec(s[n]),
					m=parseInt(l[1]),
					o=t[l[2]],
					e=k.prototype[o](e,m);
			...
			z={
				...
				m:function(a,b){
					...
					for(b%=12,a.setUTCMonth(b);
						a.getUTCMonth()!==b;)a.setUTCDate(a.getUTCDate()-1);
					return a
				},
				d:function(a,b){
		...
		formatDate:function(b,c,d){
			...
			var e={
				...
				MM:q[d].months[b.getUTCMonth()],yy:b.getUTCFullYear().toString().substring(2),
				yyyy:b.getUTCFullYear()
			};
			e.dd=(e.d<10?"0":"")+e.d,
			e.mm=(e.m<10?"0":"")+e.m,
			b=[];
			for(var f=a.extend([],c.separators),
				g=0,h=c.parts.length;h>=g;g++)f.length&&b.push(f.shift()),
				b.push(e[c.parts[g]]);
			return b.join("")
		},
		headTemplate:
